...
class ProjectStore extends EventEmitter {
  ...
  saveProject(projectId, projectData, cloudData, cloudID) {
    console.log('save');
    ...
    if(cloudID !='') {
      this._currentProjectDesc['cloudID'] = cloudID;
    }
    this._currentProjectDesc[PROJECT_THUMBNAIL] = this._dataBase64;
...
AppDispatcher.register((action) => {
  if(action.actionType == AppConstants.PROJECT_CREATE) {
    ...
    _instance.saveProject(action.projectId, action.projectData, action.cloudData, action.cloudID);
