...
const EVENT_CIRCLE_CHANGE = 'circleChange';
...
const CIRCLE_STATUS_ONE_METER = 'less-one-meter';
const CIRCLE_STATUS_HALF_METER = 'less-half-meter';

const FIRST_DISTANCE = 0.8;
const SECOND_DISTANCE = 0.8;
...
class LinkStore extends EventEmitter {
  constructor() {
    ...
    this._circleStatus = 'less-one-meter';
    this._disconnectType = '';

    this._checkBleEnabledTimer = null;
  }
  ...
  resetAutoConnect() {
    ...
    self._circleStatus = CIRCLE_STATUS_ONE_METER;
    self._disconnectType = '';
  }
  ...
  getConditionSatisfy() {
    ...
    if(self._autoDeviceList.length == 1) {
      ...
      if(self._autoConnectTimer1 >= 3) {
        self._circleStatus = CIRCLE_STATUS_HALF_METER;
        self.trigger(EVENT_CIRCLE_CHANGE);
        return true;
      }
    }
  ...
  autoConnect() {
    ...
    if(self._openScan) {
      ...
      if(condSatisfy && self._autoConnect == false && self.connectedDevice != null) {
        console.log('condSatisfy && self._autoConnect == false');
      ...
      if (self.linkType == LINKTYPE_BLE) {
        if (window.ble) {
          ...
          ble.connect(device, function() {
            ...
            self.trigger(EVENT_STATUS_CHANGE);
            this._checkBleEnabledTimer = setInterval(this.checkBleEnabled.bind(this), 1000);
          }, function(err) {
            // connect failure
            self._openScan = true;
            engine.closeDriver();
      ...
      if (window.ble) {
        if (self.connectedDevice) {
          ...
          ble.disconnect(lastConnectedDevice, () => {
            ...
            self.resetAutoConnect();
            self.status = LINK_STATUS_DISCONNECTED;
            self._circleStatus = 'less-one-meter';
            if(needTrigger == false) {
              self._disconnectType = 'tab';
              self._openScan = false;
            } else {
              self._disconnectType = '';
              self._openScan = true;
            ...
            self.trigger(EVENT_STATUS_CHANGE);
            self.trigger(EVENT_CIRCLE_CHANGE);
            clearInterval(this._checkBleEnabledTimer);
          });
        }
      }
    }
