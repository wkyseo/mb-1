...
class ColorSensorNode extends Component {
  constructor() {
    super(...arguments);
  }
  ...
  renderPreview() {
    return (
      <div className={'node-preview node-preview-shelf node-type-' + this.props.info.name} id={this.props.nodeId} data-type={this.props.info.name} data-category={this.props.info.props.category}>
        <img className="node-preview-icon" src="img/icon-colorSensor.png" />
      </div>
    );
  }
    ...
    return (
      <div className={'node-actual node-type-' + this.props.info.name} id={this.props.id} style={{
        ...
        <div className='node-body node-draggable' style={{
          background: 'url("img/electronic-colorSensor.png") center center /112px no-repeat'}}>
        </div>
