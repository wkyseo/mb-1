...
class ControlNumberInputNode extends Component {
  constructor() {
    super(...arguments);
    this.state = {
      value: ''
  ...
  openNumberInput() {
    UIActions.openNumberInputDialog((num) => {
      ...
      engine.configNode(this.props.id, {number: Number(num)});
    });
  }
      ...
      <div className={'node-actual node-type-' + this.props.info.name} id={this.props.id} style={{
        ...
        <div className='node-body node-draggable' >
          ...
          <div className="node-actual-content">
            <div className="preserve-default" {...tapOrClick(this.openNumberInput.bind(this))} ref='num' >{this.state.value}</div>
          </div>
        </div>
  ...
  componentDidMount() {
    ...
    if(nodeStore.getNodeConfigs(this.props.id)) {
      self.setState({
        value: nodeStore.getNodeConfigs(this.props.id).number.defaultValue
      });
    }
