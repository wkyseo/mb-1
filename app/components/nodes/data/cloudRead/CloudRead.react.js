...
require('./CloudRead.less');

class CloudReadNode extends Component {
  constructor() {
    super(...arguments);
    this.state = {
  ...
  renderPreview() {
    return (
      <div className={'node-preview node-preview-palette node-type-' + this.props.info.name} data-type={this.props.info.name} data-category={this.props.info.props.category}>
        <img className="node-preview-icon" src='./img/icon-cloudRead.png' />
        <span className="node-preview-name">{languages.getTranslation('cloud-read')}</span>
      </div>
    );
  }
      ...
      <div className={'node-actual node-type-' + this.props.info.name} id={this.props.id} style={{
        ...
        <div className='node-body node-draggable'>
           <div className='node-actual-body'>
            <span className="cloud-read">{languages.getTranslation('cloud-read')}</span>
          </div>       
        </div>
        <Inputs ports={this.props.info.props.in} nodeId={this.props.id} />
        <Outputs ports={this.props.info.props.out} nodeId={this.props.id} />
        <Configurator 
          title={languages.getTranslation('cloud-read')}
