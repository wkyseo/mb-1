...
class CounterNode extends Component {
  constructor() {
    super(...arguments);
    this.state = {
      value: 0
  ...
  renderPreview() {
    return (
      <div className={'node-preview node-preview-palette node-type-' + this.props.info.name} data-type={this.props.info.name} data-category={this.props.info.props.category}>
        ...
        <span className="node-preview-name">{languages.getTranslation('logic-node-counter')}</span>
      </div>
    );
  }
      ...
      <div className={'node-actual node-type-' + this.props.info.name} id={this.props.id} style={{
        ...
        <div className='node-body node-draggable'>
           <div className='node-actual-body'>
            <span className="counter-title">{languages.getTranslation('logic-node-counter')}</span>
            <span className="counter-value">{this.state.value}</span>
            <span className="counter-reset" {...tapOrClick(this.onReset.bind(this))}>{languages.getTranslation('reset')}</span>
          </div>       
        </div>
