...
class NodeTouchHandler extends TouchHandler {
  ...
  dragDeleteNode(category, top, bottom, left, right) {
    ...
    if(this._dragDeleteAble) {
      ...
      this.container.emitter.emit('showDelete', {category: tempCategory, showDelete: true});
    } else {
      // NodeUtils.removeDragDelete(this.draggingObject);
      this.container.emitter.emit('showDelete', {category: 'electronic', showDelete: false});
      this.container.emitter.emit('showDelete', {category: 'notElectronic', showDelete: false});
    }
    ...
    if (this.canMove) {
      ...
      if( this.hasMoved ){
        ...
        if (this.curConfigStatus == 'show') {
          UIActions.nodeTap(this.draggingObject.id, this.draggingObject.dataset.type);
        }
      } else {
