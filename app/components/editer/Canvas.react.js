...
class Canvas extends Component {
    ...
    return (
      <div>
        ...
        <div className="editer-canvas" id="editer-canvas-absolute"
          style={{ background: 'url("img/icon-canvas-bg.svg") repeat' }}
          ...
          <div className={'magnifier ' + (this.state.showCloudAppEntrance==true?'adjust-magnifer-position':'')} {...tapOrClick(this.magnifiy.bind(this))}>
            <img className='icon-magnifier' src='img/icon-zoom-out.png'/>
          </div>
          <div className="editer-surface-canvas"
            ref='canvas' style={{
  ...
  touchStart(e) {
    if (!e.target.closest('.preserve-default')) {
  ...
  nodeOutputChange(bundle) {
    ...
    if (item) {
      let valueTxt = item.querySelectorAll('.node-port-output .core.show .node-port-value');
      for (let i = 0; i < valueTxt.length; i++) {
        if (valueTxt[i].closest('.node-port-output').id.split('-')[1] == bundle.port) {
          ...
          } else if (bundle.value == 'true') {
            valueTxt[i].textContent = 1;
          } else if (bundle.value == 'false') {
            valueTxt[i].textContent = 0;
          } else {
            valueTxt[i].textContent = parseValue(bundle);
            /*
            let now = new Date().getTime();
            if(!valueTxt[i].dataset.lastUpdated || now - parseInt(valueTxt[i].dataset.lastUpdated) > 100) {
              valueTxt[i].dataset.lastUpdated = now;
              valueTxt[i].textContent = parseValue(bundle);
            }
            */
          }
  ...
  componentDidMount() {
    ...
    this.emitter.addListener('getCloudID', (cloudID) => {
      this.cloudProjectID = cloudID;
    });
  }
  ...
  projectBase64(baseImg) {
    projectStore.setBase64(baseImg);
    cloudAppStore.clearCloudId();
    UIActions.saveProject(projectStore.getCurrentProjectId(), this.state, this.cloudNodes.length == 0 ? null : this.cloudNodes, this.cloudProjectID);
  }
}
