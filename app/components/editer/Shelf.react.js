...
class NodeTouchHandler extends TouchHandler {
  ...
  onMove(e) {
    ...
    this._moveCount += 1;
    this.scrollNodes(e);
    if (this._isTouchNode && this.canMove && this.draggingObject) {
...
class Shelf extends Component {
        ...
        if (Node) {
          return (
            <li className='shelf-node' key={node.id}>
              <Node isPreview={true} info={this.props.nodeTypes[node.type]} nodeId={node.id}/>
            </li>
          );
        } else {
  ...
  getShelfDeleteTap() {
    ...
    if(self.state.connectedType != '') {
      ...
      deleteTips = (
        <div className='shelf-div-delete' style={{display: this._showDelete ? 'block' : 'none', width: '618px', left: '0px', borderBottomLeftRadius: '20px'}}>
          <img className='icon-drag-delete' src={'img/icon-delete-red.png'}/>
        </div>
      );
    }
